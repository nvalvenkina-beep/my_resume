...
const cardFiles = import.meta.glob("@portfolio/Карточки для экрана/*.{png,jpg,jpeg}", {
  eager: true,
  as: "url",
}) as Record<string, string>;

const portfolioFiles = import.meta.glob("@portfolio/*/*.{png,jpg,jpeg}", {
  eager: true,
  as: "url",
}) as Record<string, string>;
...
const galleryFolderAliases: Record<string, string> = {
  ...
  "Субсидирование бизнеса": "Субсидирование бизнеса",
  "Управление образовательными проектами": "Управление образовательными проектами",
  "Курс по самокоучингу состояния": "Курс по самокоучингу состояния",
  "Нейромотивация": "Нейромотивация",
  "Atlas Education": "Атлас",
  "Викингвуд": "Викингвуд",
  "Андрагогика: основы": "Андрагогика",
  "Задача. Опыт. Результат.": "Задача опыт результат",
};
...
const resolveCard = (needle: string) => {
  ...
  const match = Object.entries(cardImages).find(([name]) =>
    normalize(name).includes(normalizedNeedle),
  );
  return match?.[1] ?? "";
};

const resolveGallery = (needle: string) => {
  const alias = galleryFolderAliases[needle] ?? needle;
  if (galleryByFolderUrls[alias]) {
    return galleryByFolderUrls[alias];
  }
  const normalizedNeedle = normalize(alias);
  const entries = Object.entries(galleryByFolderUrls);
  const exactMatch = entries.find(
    ([folder]) => normalize(folder) === normalizedNeedle,
  );
  if (exactMatch) {
    return exactMatch[1];
  }
  const fuzzyMatch = entries.find(([folder]) =>
    normalize(folder).includes(normalizedNeedle),
  );
